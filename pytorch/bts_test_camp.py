...
parser.add_argument('--gpu', type=str,   help='number', default='0')
...
def test(params):
    ...
    with torch.no_grad():
        for _, sample in enumerate(tqdm(dataloader.data)):
            ...
            # Predict
            #lpg8x8, lpg4x4, lpg2x2, reduc1x1, depth_est = model(image, args.att_rank)
    ...
    for s in tqdm(range(num_test_samples)):
        ...
        if args.dataset == 'nyu':
            ...
            gt = cv2.imread(gt_path, -1).astype(np.float32)   # Visualization purpose only
            gt = gt / 255.0
            gt = (1 - gt) * 10
            gt[gt == 0] = np.amax(gt)        
        ...
        if args.dataset == 'kitti' or args.dataset == 'kitti_benchmark':
            pred_depth_scaled = (pred_depth.max()-pred_depth) * 256.0
        else:
        ...
        plt.imsave(filename_cmap_png, pred_depth_scaled, cmap='PuBu')
