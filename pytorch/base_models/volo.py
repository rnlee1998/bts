...
def _cfg(url='', **kwargs):
    return {
        'url': url,
        'num_classes': 1000, 'input_size': (3, 448, 448), 'pool_size': None,
...
class VOLO(nn.Module):
    ...
    """
    def __init__(self, layers, img_size=448, in_chans=3, num_classes=1000, patch_size=8,
    ...
    def forward_tokens(self, x):
        ...
        for idx, block in enumerate(self.network):
            if idx == 2:  # add positional encoding after outlooker blocks
                pose_embed = F.interpolate(self.pos_embed.permute(0, 3, 1, 2), size=[H//2, W//2], mode="nearest")
                x = x + pose_embed.permute(0, 2, 3, 1)
                x = self.pos_drop(x)
            x = block(x)
            if idx in [0, 2, 3, 4]:
    ...
    def forward(self, x):
        ...
        x, x_one = self.forward_embeddings(x)
        ...
        return x_one, features[0], features[1], features[2], features[3]
