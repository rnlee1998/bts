...
import numpy as np
from base_models.conformer import Conformer
def bn_init_as_tf(m):
...
class BtsModel_FCT(nn.Module):
    def __init__(self, params):
        super(BtsModel_FCT, self).__init__()

        self.encoder = Conformer(patch_size=16, channel_ratio=6, embed_dim=576, depth=12, num_heads=9, mlp_ratio=4, qkv_bias=True)
        checkpoint = torch.load("/mnt/data/liran/TransDepth-main/model/conformer_checkpoint/Conformer_base_patch16.pth",
                                map_location='cuda:{}'.format(0))
        self.encoder.load_state_dict(checkpoint)
        self.decoder = bts(params, [64, 256, 512, 1024, 2048], params.bts_size)

        self.up_1 = upconv(576, 128, ratio=4)
        self.bn_1 = nn.BatchNorm2d(128, momentum=0.01, affine=True, eps=1.1e-5)
        self.covd_1 = Conv2dReLU(384, 128, kernel_size=3)

        self.up_2 = upconv(576, 256, ratio=2)
        self.bn_2 = nn.BatchNorm2d(256, momentum=0.01, affine=True, eps=1.1e-5)
        self.covd_2 = Conv2dReLU(768, 256, kernel_size=3)

        self.transd_3 = Conv2dReLU(576, 512, kernel_size=3)
        self.covd_3 = Conv2dReLU(1536, 512, kernel_size=3)

        self.transd_4 = Conv2dReLU(576, 1024, kernel_size=3, stride=2)
        self.covd_4 = Conv2dReLU(1536, 1024, kernel_size=3)
    ...
    def forward(self, x, rank=0):
        ...
        x_0 = self.encoder.act1(x)  # 112x112x64
        features.append(x_0)

        x = self.encoder.maxpool(x_0)
        x_conv = self.encoder.conv_1(x, return_x_2=False)  # 56x56x384
        x_trans = self.encoder.trans_patch_conv(x).flatten(2).transpose(1, 2)
        x_trans = self.encoder.trans_1(torch.cat((self.encoder.cls_token.expand(B, -1, -1), x_trans), 1))  # 197x576

        x_conv, x_trans = self.encoder.conv_trans_2(x_conv, x_trans)
        x_conv, x_trans = self.encoder.conv_trans_3(x_conv, x_trans)
        x_conv, x_trans = self.encoder.conv_trans_4(x_conv, x_trans)  # 56x56X384 196X576

        x_trans_1 = x_trans[:, 1:, :].reshape(B, H//16, W//16, 576).permute(0, 3, 1, 2).contiguous()
        x_trans_1 = self.bn_1(self.up_1(x_trans_1))
        x_conv_1 = self.covd_1(x_conv)
        x_1 = torch.cat((x_trans_1, x_conv_1), 1)
        features.append(x_1)

        x_conv, x_trans = self.encoder.conv_trans_5(x_conv, x_trans)
        x_conv, x_trans = self.encoder.conv_trans_6(x_conv, x_trans)
        x_conv, x_trans = self.encoder.conv_trans_7(x_conv, x_trans)
        x_conv, x_trans = self.encoder.conv_trans_8(x_conv, x_trans)  # 28x28x768 196x576

        x_trans_2 = x_trans[:, 1:, :].reshape(B, H//16, W//16, 576).permute(0, 3, 1, 2).contiguous()
        x_trans_2 = self.bn_2(self.up_2(x_trans_2))
        x_conv_2 = self.covd_2(x_conv)
        x_2 = torch.cat((x_trans_2, x_conv_2), 1)
        features.append(x_2)

        x_conv, x_trans = self.encoder.conv_trans_9(x_conv, x_trans)
        x_conv, x_trans = self.encoder.conv_trans_10(x_conv, x_trans)
        x_conv, x_trans = self.encoder.conv_trans_11(x_conv, x_trans)  # 14x14x1536 196x576

        x_trans_3 = x_trans[:, 1:, :].reshape(B, H//16, W//16, 576).permute(0, 3, 1, 2).contiguous()
        x_trans_3 = self.transd_3(x_trans_3)
        x_conv_3 = self.covd_3(x_conv)
        x_3 = torch.cat((x_trans_3, x_conv_3), 1)
        features.append(x_3)

        x_conv, x_trans = self.encoder.conv_trans_12(x_conv, x_trans)  # 7x7
        x_trans_4 = x_trans[:, 1:, :].reshape(B, H//16, W//16, 576).permute(0, 3, 1, 2).contiguous()
        x_trans_4 = self.transd_4(x_trans_4)
        x_conv_4 = self.covd_4(x_conv)
        x_4 = torch.cat((x_trans_4, x_conv_4), 1)
        features.append(x_4)
