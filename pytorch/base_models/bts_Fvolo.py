...
import numpy as np
from base_models.volo import volo_d3
def bn_init_as_tf(m):
...
class BtsModel_FVolo(nn.Module):
    def __init__(self, params):
        super(BtsModel_FVolo, self).__init__()

        self.encoder = volo_d3()
        # checkpoint = torch.load("/mnt/data/shaoshuwei/TransDepth-main/model/volo_checkpoint/d3_448_86.3.pth.tar",
                                # map_location='cuda:{}'.format(params.gpu))
        checkpoint = torch.load("/mnt/data/liran/TransDepth-main/model/volo_checkpoint/d3_448_86.3.pth.tar",
                                map_location='cuda:{}'.format(0))  
        self.encoder.load_state_dict(checkpoint)
        self.decoder = bts(params, [64, 256, 512, 1024, 2048], params.bts_size)

        self.up_1 = upconv(256, 256, ratio=2)
        self.bn_1 = nn.BatchNorm2d(256, momentum=0.01, affine=True, eps=1.1e-5)

        self.up_2 = upconv(512, 512, ratio=2)
        self.bn_2 = nn.BatchNorm2d(512, momentum=0.01, affine=True, eps=1.1e-5)

        self.covd_3 = Conv2dReLU(512, 1024, kernel_size=3)

        self.covd_4 = Conv2dReLU(512, 2048, kernel_size=3, stride=2)
    ...
    def forward(self, x, rank=0):
        ...
        features[2] = self.bn_1(features[2])

        features[3] = self.up_2(features[3])
        features[3] = self.bn_2(features[3])

        features[4] = self.covd_3(features[4])

        features[5] = self.covd_4(features[5])
